...
#[test]
fn pivot_camera_distance_is_clamped_by_runtime_terrain_and_can_fail_closed() {
    let mut bridge = init_bridge();
    bridge
    ...
    assert!(constrained.terrain_constrained);
    assert!(constrained.after.distance.unwrap() < 8.0);
    assert!(constrained.after.snapshot.pose.position[2] < 4.5);

    let blocked = bridge
        .apply_camera_mode_command(CameraModeCommand {
            camera,
            expected_revision: constrained.after.revision,
            target: CameraModeTarget::Orbit {
                pivot: [0.0, 1.0, 0.0],
                distance: 8.0,
                min_distance: 5.0,
    ...
    assert_eq!(
        blocked.rejection,
        Some(CameraControllerRejection::TerrainBlocked)
    );
    assert_eq!(blocked.before, constrained.after);
    assert_eq!(blocked.after, constrained.after);
}

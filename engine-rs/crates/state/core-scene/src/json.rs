...
fn decode_kind(j: &Json) -> Result<SceneNodeKind, SceneDecodeError> {
    ...
    match tag {
        "emptyGroup" => Ok(SceneNodeKind::EmptyGroup),
        "staticMesh" => Ok(SceneNodeKind::StaticMesh(asset()?)),
        "sprite" => Ok(SceneNodeKind::Sprite(asset()?)),
        "voxelVolume" => Ok(SceneNodeKind::VoxelVolume(asset()?)),

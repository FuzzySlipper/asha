...
- Provider lanes are split (`test:reference-provider` vs `test:rust-provider`), so "the TS slice becomes the behavioral golden" is structurally prevented rather than merely warned about. `docs/runtime-session-facade.md` states plainly that the non-claims "no longer mean the FPS demo owns local combat/health/target authority."

...
- Canonical ProjectContent may declare one bounded `inputCatalog` extension. Rust project admission composes its consumer-namespaced actions and bindings with protected Engine defaults before runtime publication, and fresh RuntimeSessions resolve the stored binding without a consumer or Engine rebuild. Browser/game composition consumes the resulting semantic `ResolvedInputAction` through `BrowserInputHost` or the renderer-host callback; it must not decode stored input JSON, install a mutable action registry, or branch gameplay on DOM controls.
...
- #6089 adds the generated `ProjectInputCatalog` and `inputCatalog` ProjectContent kind. Rust admits at most one bounded consumer-namespaced extension, rejects reserved namespaces and protected Engine control replacement, and composes admitted actions into each fresh input Session. `BrowserInputHost` keeps the same normalization/readout path, while `@asha/renderer-host` adds an optional semantic `onResolvedAction` callback for game composition. The compatibility markers remain `contracts.v0`, `runtime-bridge.v0`, and `renderer-host.v1` because these surfaces are additive; rebuilding generated contracts and the native bridge together remains required.

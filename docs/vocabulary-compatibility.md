...
| `runtime_bridge_api::WorldLoadRequest`, TS `WorldLoadRequest` | `ProjectBundleLoadRequest` or `RuntimeSessionLoadRequest` | `runtime-bridge-api`, `@asha/runtime-bridge` / rust-bridge and ts-shell | Native bridge glue, runtime bridge mock/native/launcher, conformance tests, `@asha/devtools` bundle actions | Future wrapper is the smallest first code step. Full rename is blocked by bridge manifest and generated native operation compatibility. | `asha-demo` skeleton should not call this until the public-surface policy says which facade is allowed. If it appears, docs should frame it as compatibility request for loading a ProjectBundle into a RuntimeSession. |
| Bridge verbs `load_world_bundle` / `loadWorldBundle`, `save_current_world` / `saveCurrentWorld`, `get_composition_status` / `getCompositionStatus`, `unload_world` | `loadProjectBundle`, `saveProjectBundle` or `saveSession`, `getSessionCompositionStatus`, `unloadSession` | `runtime-bridge-api`, `native-bridge`, `@asha/runtime-bridge` | Bridge manifest, native fail-closed checks, runtime bridge generated operation tables, launcher, smoke/native checks | Alias/wrapper first, then rename only with bridge-manifest and native compatibility plan. Current verbs are stable bridge surface. | Demo code must use only package-root runtime facade methods. New demo docs should say "load ProjectBundle into RuntimeSession" and note any current method name as compatibility. |
...
1. Add root-barrel TS compatibility aliases in `@asha/runtime-bridge` for target
   vocabulary DTO names, for example `ProjectBundleLoadRequest` as an alias of
   current `WorldLoadRequest`, with tests proving no generated-file import path is
   required. Do not rename bridge manifest verbs yet.
2. Add documentation comments in `runtime-bridge-api` and `@asha/runtime-bridge`
   that describe `loadWorldBundle` as the current compatibility method for
   "load ProjectBundle into RuntimeSession" semantics.
3. Plan a contract-steward protocol migration for `protocol-world-bundle` to a
...
4. Plan a rust-state/rust-rule split for the broader `RuntimeSession` model: keep
...
```sh
rg -n "\\bWorldState\\b|\\bWorldBundleManifest\\b|\\bWorldLoadRequest\\b|\\bWorldSaveSummary\\b|\\bloadWorldBundle\\b|\\bload_world_bundle\\b|\\bsaveCurrentWorld\\b|\\bsave_current_world\\b|\\bgetCompositionStatus\\b|\\bget_composition_status\\b|\\bWorldSection\\b|\\bLoadStage::WorldStateSnapshot\\b|restoreWorldState|world-bundle|world bundle" \

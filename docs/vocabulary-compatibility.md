...
| `LoadStage::WorldStateSnapshot` / `restoreWorldState` in serialization/rule load plans | `restoreSessionState` or `restoreSpatialSessionState` | `svc-serialization`, `rule-world-bundle` | Load-plan fixtures, devtools bundle panel | The generated ProjectBundle contract now uses `sessionStateSnapshot` / `restoreSessionState`; lower serialization/rule fixtures still carry old wire labels until their task lands. | Demo evidence should say "runtime/session restore" in prose. |
| `runtime_bridge_api::ProjectBundleLoadRequest`, TS `ProjectBundleLoadRequest` | Current target bridge DTO name | `runtime-bridge-api`, `@asha/runtime-bridge` / rust-bridge and ts-shell | Native bridge glue, runtime bridge mock/native/launcher, conformance tests, `@asha/devtools` bundle actions | Migrated in the bridge/native lane. Do not add compatibility aliases back to the old request names. Remaining legacy work is protocol/source crate naming such as `protocol-project-bundle`. | `asha-demo` should call package-root runtime facade methods and describe the behavior as loading a ProjectBundle into a RuntimeSession. |
...
1. Plan a rust-state/rust-rule split for the broader `RuntimeSession` model: keep
...
```sh
rg -n "\\bWorldState\\b|\\bProjectBundleManifest\\b|\\bProjectBundleLoadRequest\\b|\\bProjectBundleSaveSummary\\b|\\bloadProjectBundle\\b|\\bload_project_bundle\\b|\\bsaveProjectBundle\\b|\\bsave_project_bundle\\b|\\bgetProjectBundleCompositionStatus\\b|\\bget_project_bundle_composition_status\\b|\\bWorldSection\\b|\\bLoadStage::WorldStateSnapshot\\b|restoreWorldState|project-bundle|project bundle" \

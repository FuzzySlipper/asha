...
import { auditTsSourceShapePolicy } from './ts-source-shape-policy-audit.mjs';

const POLICY_REL = 'harness/depgraph/ts-source-shape-policy.json';
...
function readPolicyAtRef(ref) {
  ...
  if (result.status !== 0) {
    throw new Error(`cannot read TypeScript source-shape policy at ${ref}: ${result.stderr.trim()}`);
  }
...
if (basePolicyPath !== undefined) {
  ...
  if (selectedBaseRef === undefined) {
    console.log('TypeScript source-shape policy audit skipped: no base revision is available.');
    process.exit(0);
  }
...
const failures = [];
auditTsSourceShapePolicy(basePolicy, currentPolicy, failures);
...
console.log(`TypeScript source-shape policy audit: OK (base ${baseLabel})`);

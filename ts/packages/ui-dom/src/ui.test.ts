...
void test('HUD menu controls map to typed intents only', () => {
  ...
  assert.deepEqual(
    hudIntentToTimeControlCommand({ kind: 'ui.pause_intent', source: 'hud_menu' }),
    { operation: 'pause' },
  );
  assert.deepEqual(
    hudIntentToTimeControlCommand({ kind: 'ui.resume_intent', source: 'hud_menu' }),
    { operation: 'resume' },
  );
  assert.equal(
    hudIntentToTimeControlCommand({ kind: 'ui.open_options_intent', source: 'hud_menu' }),
    null,
  );

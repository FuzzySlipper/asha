...
export function buildPolicyRunSummary(input) {
    let accepted = 0;
    ...
    const rows = input.executions.map((e) => ({
        policyId: e.policyId,
        version: e.version,
...
export function formatPolicyRunSummary(view) {
    const lines = [
    ...
    for (const row of view.rows) {
        lines.push(`  policy ${row.policyId} v${row.version} proposed=${row.proposedCount} violations=${row.violationCount}`);
    }
...
//# sourceMappingURL=policy-panel.js.map
import {
  consumerCompatibilityDiagnostic,
  type AshaCompatibilitySurfaceMetadata,
  type AshaConsumerCompatibilityDiagnostic,
  type AshaConsumerCompatibilityMetadata,
  type AshaConsumerCompatibilityValidation,
  type AshaGameManifest,
  type AshaProtocolCompatibilityMetadata,
  ...
  if (manifestVersion !== metadataVersion) {
    diagnostics.push(
      consumerCompatibilityDiagnostic(
        ...
        path,
        `manifest declares "${manifestVersion}" but ASHA metadata provides "${metadataVersion}"`,
      ),
    );
  }
}

...
function bindings(): GameplayModuleBindingRegistry {
  const module: GameplayModuleRef = {
...
void test('public prefab authoring creates edits places and inspects a multi-part definition', () => {
  ...
  if (!edited.ok) throw new Error('edit should succeed');
  state = edited.state;
  ...
  for (const command of [
    ...
    if (!placed.ok) throw new Error('placement should succeed');
    state = placed.state;
  }

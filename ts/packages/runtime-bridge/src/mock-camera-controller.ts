...
function state(
  revision: number,
  mode: CameraControllerState['mode'],
  pivot: CameraControllerState['pivot'],
...
function rejectedMode(
  before: CameraControllerState,
  rejection: CameraControllerRejection,
): CameraModeChangeReceipt {
  return {
    accepted: false,
    before,
...
export class MockCameraControllers {
  readonly #states = new Map<number, CameraControllerState>();
  ...
  syncFirstPerson(snapshot: CameraSnapshot): void {
    ...
    if (!before || before.mode !== 'firstPerson') return;
    this.#states.set(snapshot.camera, state(
      before.revision + 1,
      'firstPerson',
      null,
      null,
  ...
  applyMode(command: CameraModeCommand): CameraModeChangeReceipt | undefined {
    ...
    if (command.expectedRevision !== before.revision) return rejectedMode(before, 'staleRevision');
    ...
    let mode: CameraControllerState['mode'];
    let pivot: CameraControllerState['pivot'];
    let distance: number | null;
    let minDistance: number | null;
    let maxDistance: number | null;
    let pose: CameraPose;
    if (target.mode === 'firstPerson') {
      if (!finitePose(target.pose)) return rejectedMode(before, 'invalidTarget');
      mode = 'firstPerson';
      pivot = null;
      distance = null;
      minDistance = null;
      ...
      if (!validMetric(target.pivot, target.distance, target.minDistance, target.maxDistance)
        ...
        || target.pitchDegrees > 89) return rejectedMode(before, 'invalidTarget');
      mode = 'orbit';
      pivot = target.pivot;
      ...
      if (!validMetric(target.pivot, target.height, target.minHeight, target.maxHeight)
        ...
        || !Number.isFinite(target.pitchDegrees)
        || target.pitchDegrees < -89
        || target.pitchDegrees > -30) return rejectedMode(before, 'invalidTarget');
      mode = 'topDown';
      pivot = target.pivot;
    ...
    const receipt: CameraModeChangeReceipt = {
      accepted: true,
      before,
      after,
      transition,
  ...
  applyNavigation(input: CameraNavigationInputEnvelope): CameraNavigationReceipt | undefined {
    ...
    if (!before) return undefined;
    if (input.expectedRevision !== before.revision) return rejectedNavigation(before, 'staleRevision');
    if (before.mode === 'firstPerson') return rejectedNavigation(before, 'incompatibleMode');
    ...
    const receipt: CameraNavigationReceipt = {
      accepted: true,
      before,
      after,
      terrainConstrained: false,

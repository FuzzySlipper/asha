...
} satisfies CameraNavigationInputEnvelope;

export function createNativeCameraControllerHandlers(
  calls: string[],
  hashA: string,
  hashB: string,
  hashC: string,
): Pick<NativeAddon, 'applyCameraModeCommand' | 'applyCameraNavigationInput' | 'readCameraControllerState'> {
  const firstPersonController = (): CameraControllerState => ({
    schemaVersion: 1,
    revision: 0,
    camera: CAMERA_INPUT.camera,
    mode: 'firstPerson',
  ...
  const orbitController = (): CameraControllerState => ({

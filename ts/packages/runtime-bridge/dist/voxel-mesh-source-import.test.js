...
import { NativeRuntimeBridge, RuntimeBridgeError, createRuntimeSessionFacade, } from './index.js';
import { MockRuntimeBridge } from './mock.js';
import { VOXEL_CONVERSION_MESH_SOURCE_IMPORT_REQUEST, createNativeVoxelMeshSourceHandlers, importedMeshReceipt, } from './native-voxel-mesh-source.test-fixture.js';
const nativeRootRequest = VOXEL_CONVERSION_MESH_SOURCE_IMPORT_REQUEST;
...
void test('reference RuntimeSession and mock RuntimeBridge fail closed for mesh source import', () => {
    ...
    assert.throws(() => mock.importVoxelConversionMeshSource(VOXEL_CONVERSION_MESH_SOURCE_IMPORT_REQUEST), (error) => error instanceof RuntimeBridgeError && error.kind === 'not_initialized');
    mock.initializeEngine({ seed: 5 });
    assert.throws(() => mock.importVoxelConversionMeshSource(VOXEL_CONVERSION_MESH_SOURCE_IMPORT_REQUEST), (error) => error instanceof RuntimeBridgeError && error.kind === 'operation_unimplemented');
    const session = createRuntimeSessionFacade({
        bridge: new MockRuntimeBridge(),
        mode: 'reference',
    });
    session.initialize(sessionInput());
    assert.throws(() => session.importVoxelConversionMeshSource(VOXEL_CONVERSION_MESH_SOURCE_IMPORT_REQUEST), (error) => error instanceof RuntimeBridgeError && error.kind === 'operation_unimplemented');
});
//# sourceMappingURL=voxel-mesh-source-import.test.js.map
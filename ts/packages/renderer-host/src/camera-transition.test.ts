...
function snapshot(position: readonly [number, number, number], yawDegrees: number): CameraSnapshot {
  return {
    camera: 1 as CameraSnapshot['camera'],
    tick: 4,
...
void test('camera transition sampling is disposable and preserves exact authority endpoints', () => {
  ...
  const transition: CameraTransitionReadout = {
    from,
    to,
...
void test('camera transition sampling rejects malformed renderer timing', () => {
  const value: CameraTransitionReadout = {
